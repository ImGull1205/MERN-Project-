...
export default function PlacesFormPage() {
  ...
  const [features,setFeatures] = useState(placeData?.features?.split(', ') || []);
  const [extraInfo,setExtraInfo] = useState(placeData?.extraInfo || '');
  const [locationData, setLocationData] = useState(placeData?.location || {
    province: '',
    district: '',
    street: '',
    houseNumber: '',
  });
  const [tickets, setTickets] = useState(placeData?.tickets || [{ type: '', price: '' }]);
  ...
  const inputClass = "w-full p-3 rounded-lg bg-gray-800 text-white border border-gray-700 focus:border-app_blue focus:outline-none";
  const buttonClass = "bg-app_blue text-white px-6 py-2 rounded-lg hover:bg-blue-700 transition-colors";

  return (
    <div className="min-h-screen bg-gray-900 text-white p-8">
      <form onSubmit={savePlace} className="max-w-4xl mx-auto space-y-6">
        <div className="p-2 flex justify-center item-center">
          <Link to="/" className="bg-app_blue rounded-full py-2.5 px-4 inline-flex items-center gap-3 w-full justify-center">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6 text-app_yellow shrink-0">
        ...
        <div className="space-y-4">
          ...
          {tickets.map((ticket, index) => (
            <div key={index} className="grid grid-cols-[1fr,1fr,auto] items-center gap-4">
              <input
                type="text"
                value={ticket.type}
                onChange={ev => handleTicketChange(index, 'type', ev.target.value)}
              ...
              <input
                type="number"
                value={ticket.price}
                onChange={ev => handleTicketChange(index, 'price', ev.target.value)}
                placeholder="Price"
                className={inputClass}
              />
              <DeleteOutlined 
